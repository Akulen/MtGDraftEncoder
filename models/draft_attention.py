...
from models.attention import (
    MultiheadAttentionLayer, AlternatingResidualAttention
)
from data_types import Cards, Sets, Drafts

class DraftTransformer2(DraftWRPredictor):
    ...
    def __init__(
        ...
        self.set_context = SetContext(
            ...
            layer_fns=[
                lambda subkey: MultiheadAttentionLayer(
                    key=subkey,
                    d_model=d_model,
                    num_heads=num_heads,
                    dropout_p=dropout_p
        ...
        self.predictor_head = eqx.nn.MLP(
            ...
            depth=pred_n_layers,
            activation=jax.nn.gelu, #jax.nn.leaky_relu,
            final_activation=jax.nn.sigmoid,
            key=key_predict
        )
        self.dropout = eqx.nn.Dropout(dropout_p)

    def get_context_encodings(
        self,
        key: PRNGKeyArray,
        cards: Cards,
        sets: Sets,
        ...
        h_cards_set, state = self.set_context(
            key, cards, sets, drafts, rank_embedding, state, inference
        )
        return h_cards_set
    ...
    def __call__(
        ...
        h_cards_set, state = self.set_context(
            subkey, cards, sets, drafts, rank_embedding, state, inference
        )
