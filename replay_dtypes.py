...
COLUMN_TYPES = (
    # Card Index
    (re.compile(r"^id$"), "int32"),
    (re.compile(r"^name$"), "str"),
    (re.compile(r"^rarity$"), "str"),
    (re.compile(r"^color_identity$"), "str"),
    (re.compile(r"^mana_value$"), "int8"),
    (re.compile(r"^types$"), "str"),
    (re.compile(r"^is_booster$"), "bool"),
    # Metadata
...
def get_dtypes(
    ...
    dtypes: Mapping[str, PolarsDataType] = {}
    print_missing = True
    for column in pl.read_csv(filename, n_rows=0).columns:
        for regex, column_type in COLUMN_TYPES:
            if regex.match(column):
                dtypes[column] = dtype_mapping[column_type]
                break
        else:
            if print_missing:
